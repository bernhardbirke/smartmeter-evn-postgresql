...
while 1:
    ...
    try:
        ...
        WirkenergieP = int(str(results_32[0].get(
            'value')), 16)*10**s8(str(results_int8[0].get('value')))
        WirkenergiePUnit = units[int(results_enum[0].get('value'), 16)]

        # Wirkenergie A- in Wattstunden
        WirkenergieN = int(str(results_32[1].get(
            'value')), 16)*10**s8(str(results_int8[1].get('value')))
        WirkenergieNUnit = units[int(results_enum[1].get('value'), 16)]

        # Momentanleistung P+ in Watt
        MomentanleistungP = int(str(results_32[2].get(
            'value')), 16)*10**s8(str(results_int8[2].get('value')))
        MomentanleistungPUnit = units[int(results_enum[2].get('value'), 16)]

        # Momentanleistung P- in Watt
        MomentanleistungN = int(str(results_32[3].get(
            'value')), 16)*10**s8(str(results_int8[3].get('value')))
        MomentanleistungNUnit = units[int(results_enum[3].get('value'), 16)]

        # Spannung L1
        SpannungL1 = int(str(results_16[0].get(
            'value')), 16)*10**s8(str(results_int8[4].get('value')))
        SpannungL1Unit = units[int(results_enum[4].get('value'), 16)]

        # Spannung L2
        SpannungL2 = int(str(results_16[1].get(
            'value')), 16)*10**s8(str(results_int8[5].get('value')))
        SpannungL2Unit = units[int(results_enum[5].get('value'), 16)]

        # Spannung L3
        SpannungL3 = int(str(results_16[2].get(
            'value')), 16)*10**s8(str(results_int8[6].get('value')))
        SpannungL3Unit = units[int(results_enum[6].get('value'), 16)]

        # Strom L1
        StromL1 = int(str(results_16[3].get('value')), 16) * \
            10**s8(str(results_int8[7].get('value')))
        StromL1Unit = units[int(results_enum[7].get('value'), 16)]

        # Strom L2
        StromL2 = int(str(results_16[4].get('value')), 16) * \
            10**s8(str(results_int8[8].get('value')))
        StromL2Unit = units[int(results_enum[8].get('value'), 16)]

        # Strom L3
        StromL3 = int(str(results_16[5].get('value')), 16) * \
            10**s8(str(results_int8[9].get('value')))
        StromL3Unit = units[int(results_enum[9].get('value'), 16)]

        # Leistungsfaktor
        Leistungsfaktor = s16(str(results_int16[0].get(
            'value')))*10**s8(str(results_int8[10].get('value')))
        ...
        if printValue:
            ...
            print('Momentanleistung+: ' +
                  str(MomentanleistungP) + MomentanleistungPUnit)
            print('Momentanleistung-: ' +
                  str(MomentanleistungN) + MomentanleistungNUnit)
            ...
            print('Momentanleistung: ' + str(MomentanleistungP -
                  MomentanleistungN) + MomentanleistungPUnit)
            print()
            print()
